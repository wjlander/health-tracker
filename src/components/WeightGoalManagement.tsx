...
export const WeightGoalManagement: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Set current weight for display and as start weight for new goals (prioritize Fitbit)
      let latestWeight = 0;
      if (fitbitResult?.weight?.weight) {
        ...
        latestWeight = convertWeight(healthData[0].weight, 'lbs', weightUnit);
      }
      
      setCurrentWeight(latestWeight);
      
      if (latestWeight > 0) {
        setFormData(prev => ({ ...prev, start_weight: latestWeight }));
      }
  ...
  const calculateProgress = (goal: any) => {
    if (currentWeight === 0) return { progress: 0, percentage: 0, onTrack: false, currentWeight: 0, targetWeight: 0 };
    ...
    const displayCurrentWeight = currentWeight || startWeight;
    
    const progress = displayCurrentWeight - startWeight;
    ...
    return { progress, percentage, onTrack, currentWeight: displayCurrentWeight, targetWeight };
  };
