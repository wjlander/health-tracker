      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-blue-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <div className="text-sm text-gray-600 space-y-1">
              <div>Manual: {combinedHistory[combinedHistory.length - 1] ? formatWeight(combinedHistory[combinedHistory.length - 1].weight, weightUnit) : 'No data'}</div>
              {fitbitData?.weights?.length > 0 && (
                <div>Fitbit: {formatWeight(convertWeight(fitbitData.weights[fitbitData.weights.length - 1]?.weight || 0, 'lbs', weightUnit), weightUnit)}</div>
              )}
            </div>
          </div>
        </div>
