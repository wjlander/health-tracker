/*
  # Final Clean Database Creation for Health Tracker

  This migration creates a clean database schema with all required tables:
  ...
  Each table includes proper RLS policies and indexes.
*/
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'users' AND constraint_name = 'users_name_key'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'health_entries' AND constraint_name = 'health_entries_user_id_date_key'
    ) THEN
        ALTER TABLE health_entries ADD CONSTRAINT health_entries_user_id_date_key UNIQUE (user_id, date);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'user_integrations' AND constraint_name = 'user_integrations_user_id_provider_key'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'fitbit_activities' AND constraint_name = 'fitbit_activities_user_id_date_key'
    ) THEN
        ALTER TABLE fitbit_activities ADD CONSTRAINT fitbit_activities_user_id_date_key UNIQUE (user_id, date);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'fitbit_weights' AND constraint_name = 'fitbit_weights_user_id_date_key'
    ) THEN
        ALTER TABLE fitbit_weights ADD CONSTRAINT fitbit_weights_user_id_date_key UNIQUE (user_id, date);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'fitbit_foods' AND constraint_name = 'fitbit_foods_user_id_date_key'
    ) THEN
        ALTER TABLE fitbit_foods ADD CONSTRAINT fitbit_foods_user_id_date_key UNIQUE (user_id, date);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'fitbit_sleep' AND constraint_name = 'fitbit_sleep_user_id_date_key'
    ) THEN
        ALTER TABLE fitbit_sleep ADD CONSTRAINT fitbit_sleep_user_id_date_key UNIQUE (user_id, date);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'daily_nutrition_summary' AND constraint_name = 'daily_nutrition_summary_user_id_date_key'
    ) THEN
        ALTER TABLE daily_nutrition_summary ADD CONSTRAINT daily_nutrition_summary_user_id_date_key UNIQUE (user_id, date);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'menstrual_entries' AND constraint_name = 'menstrual_entries_user_id_date_key'
    ) THEN
        ALTER TABLE menstrual_entries ADD CONSTRAINT menstrual_entries_user_id_date_key UNIQUE (user_id, date);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'premenopausal_entries' AND constraint_name = 'premenopausal_entries_user_id_date_key'
    ) THEN
        ALTER TABLE premenopausal_entries ADD CONSTRAINT premenopausal_entries_user_id_date_key UNIQUE (user_id, date);
    END IF;
END $$;
...
DO $$
DECLARE
    jayne_exists boolean;
    william_exists boolean;
BEGIN
    -- Check if Jayne exists
    SELECT EXISTS(SELECT 1 FROM users WHERE name = 'Jayne') INTO jayne_exists;
    IF NOT jayne_exists THEN
        INSERT INTO users (name, role) VALUES ('Jayne', 'patient');
        RAISE NOTICE 'Created user: Jayne';
    ELSE
        RAISE NOTICE 'User Jayne already exists';
    END IF;
    
    -- Check if William exists
    SELECT EXISTS(SELECT 1 FROM users WHERE name = 'William') INTO william_exists;
    IF NOT william_exists THEN
        INSERT INTO users (name, role) VALUES ('William', 'patient');
        RAISE NOTICE 'Created user: William';
    ELSE
        RAISE NOTICE 'User William already exists';
    END IF;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Database is ready for health tracking!';
END $$;