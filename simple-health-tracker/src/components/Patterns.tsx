...
export const Patterns: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [healthResult, fitbitResult] = await Promise.all([
      ...
      if (healthResult.data) {
        ...
        calculateCorrelations(healthResult.data, fitbitResult);
      }
  ...
  const calculateCorrelations = (health: any[], fitbit: any) => {
    if (health.length < 3) {
      setCorrelationData([]);
      return;
    }
